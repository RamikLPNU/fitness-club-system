...
function loadClientActivity() {
  fetch(`${API_BASE}/reports/client-activity`)
    .then(res => res.json())
    .then(data => {
      ...
      data.forEach(c => {
        ...
        li.textContent = `${c.client} — ${c.visits} відвідувань`;
        list.appendChild(li);
      });
    })
    .catch(err => console.error("Client activity error:", err));
}

document.getElementById("toggleClientActivity").addEventListener("click", () => {
  const list = document.getElementById("clientActivityList");
  const btn = document.getElementById("toggleClientActivity");

  if (list.classList.contains("collapsed")) {
    list.classList.remove("collapsed");
    list.classList.add("expanded");
    btn.textContent = "Приховати список";
  } else {
    list.classList.remove("expanded");
    list.classList.add("collapsed");
    btn.textContent = "Показати список";
  }
});
